...
class TsUtil {
    ...
    addCat(pid, objCat) {
        if (pid !== 0x01)
            return false;
        if (objCat.table_id !== 0x01)
            return false;
        if (objCat.current_next_indicator === 0)
            return false;
        return true;
    }
    addPmt(pid, objPmt) {
        if (objPmt.table_id !== 0x02)
            return false;
        if (objPmt.current_next_indicator === 0)
            return false;
        return true;
    }
    addDsmcc(pid, objDsmcc) {
        if (objDsmcc.table_id !== 0x3B && objDsmcc.table_id !== 0x3C)
            return false;
        if (objDsmcc.current_next_indicator === 0)
            return true;
        ...
        if (!updateSection(subTable, objDsmcc))
            return false;
        switch (objDsmcc.table_id) {
                ...
                for (let module of objDii.modules) {
                    ...
                    for (let descriptor of module.moduleInfo) {
                        switch (descriptor.descriptor_tag) {
                            case 0x01:
                                _module.type = new char_1.default(descriptor.text_char);
                                break;
                            case 0x02:
                                _module.name = new char_1.default(descriptor.text_char);
                                break;
                            case 0x03:
                                _module.info = new char_1.default(descriptor.text_char);
                                break;
                        }
                    }
                }
            ...
            case 0x3C: {
                ...
                if (objDdb.moduleVersion !== module.moduleVersion)
                    return false;
                if (module.blocks.hasOwnProperty(objDdb.blockNumber))
                    return false;
                module.blocks[objDdb.blockNumber] = objDdb.blockDataByte;
                module.blockCount++;
                if (module.blockCount === module.numberOfBlocks) {
                    let dataModuleByte = buffer_1.Buffer.concat(module.blocks);
                        ...
                        switch (_module.name) {
                            ...
                            case "CS_LOGO-05": {
                                ...
                                for (let logo of dataModule.logos) {
                                    ...
                                    if (original_network_ids.size === 0)
                                        continue;
                                    for (let original_network_id of original_network_ids) {
                                        ...
                                        if (logo_id.flag)
                                            continue;
                                        let logoData = new logo_1.default(logo.data_byte).decode();
    ...
    addNit(pid, objNit) {
        if (pid !== 0x10)
            return false;
        if (objNit.table_id !== 0x40 && objNit.table_id !== 0x41)
            return false;
        if (objNit.current_next_indicator === 0)
            return false;
        ...
        if (!updateSection(subTable, objNit))
            return false;
    ...
    addSdt(pid, objSdt) {
        if (pid !== 0x11)
            return false;
        if (objSdt.table_id !== 0x42 && objSdt.table_id !== 0x46)
            return false;
        if (objSdt.current_next_indicator === 0)
            return false;
        ...
        if (!updateSection(subTable, objSdt))
            return false;
            ...
            for (let descriptor of service.descriptors) {
                switch (descriptor.descriptor_tag) {
                    case 0x48:
                        ...
                        _service.service.service_provider_name = new char_1.default(descriptor.service_provider_name_char).decode();
                        _service.service.service_name = new char_1.default(descriptor.service_name_char).decode();
                        break;
                    case 0xCF:
    ...
    addBat(pid, objBat) {
        if (pid !== 0x11)
            return false;
        if (objBat.table_id !== 0x4A)
            return false;
        if (objBat.current_next_indicator === 0)
            return true;
        return true;
    }
    addEit(pid, objEit) {
    ...
    addTdt(pid, objTdt) {
        if (pid !== 0x14)
            return false;
        if (objTdt.table_id !== 0x70)
            return false;
        this.time = new date_1.default(objTdt.JST_time).decode();
        return true;
    }
    addTot(pid, objTot) {
        if (pid !== 0x14)
            return false;
        if (objTot.table_id !== 0x73)
            return false;
        this.time = new date_1.default(objTot.JST_time).decode();
        return true;
    }
    addSdtt(pid, objSdtt) {
        if (pid !== 0x23 && pid !== 0x28)
            return false;
        if (objSdtt.table_id !== 0xC3)
            return false;
        if (objSdtt.table_id_ext >> 8 !== 0xFF)
            return false;
        if (objSdtt.current_next_indicator === 0)
            return false;
        let subTable = getNestedObject(this.versions.sdtt, [objSdtt.table_id, objSdtt.table_id_ext]);
        updateSubTable(subTable, objSdtt);
        if (!updateSection(subTable, objSdtt))
            return false;
    ...
    addCdt(pid, objCdt) {
        if (pid !== 0x29)
            return false;
        if (objCdt.table_id !== 0xC8)
            return false;
        if (objCdt.current_next_indicator === 0)
            return false;
        ...
        if (!updateSection(subTable, objCdt))
            return false;
        ...
        switch (objCdt.data_type) {
            case 0x01: {
                ...
                if (logo_id.flag)
                    return false;
                let logoData = new logo_1.default(dataModule.data_byte).decode();
                logo_id.content.set(dataModule.logo_type, logoData);
                if (logo_id.content.size === 6) {
                    logo_id.flag = true;
                }
    ...
    hasTransportStreams(onid) {
        ...
        if (!checkNestedObject(this.transport_streams, [onid]))
            return false;
        let transport_streams = getNestedObject(this.transport_streams, [onid]);
        if (!transport_streams.flag)
            return false;
        return true;
    }
    hasServices(onid, tsid) {
        ...
        if (!checkNestedObject(this.services, [onid, tsid]))
            return false;
    ...
    hasLogo(logo_id, onid) {
        if (!checkNestedObject(this.logo_ids, [onid, logo_id]))
            return false;
    ...
    getLogoId(onid, tsid, sid) {
        if (!checkNestedObject(this.logo_id_refs, [onid, tsid, sid]))
            return null;
