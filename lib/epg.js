...
const epg_table_1 = require("./epg_table");
class TsEpg {
    epg;
    constructor() {
        this.epg = new Map();
    ...
    addEit(pid, objEit, time) {
        if (pid !== 0x12)
            return false;
        if (objEit.current_next_indicator === 0)
            return false;
            ...
            for (let target_flags of [objService.basic_flags, objService.extended_flags]) {
                for (let i = 0; i < 0x08; i++) {
                    target_flags.flags.push({
                        flag: buffer_1.Buffer.alloc(0x20, 0x00),
                        ignore: buffer_1.Buffer.alloc(0x20, 0xFF),
                        version_number: -1
                    });
                }
            }
        ...
        for (let event of objEit.events) {
            ...
            if (objService.schedule.hasOwnProperty(event.event_id)) {
                ...
                if (this.needUpdate(objEvent, event)) {
                    let startTime = new date_1.default(event.start_time).decode();
                    let duration = new date_1.default(event.duration).decodeTime();
            ...
            for (let descriptor of event.descriptors) {
                switch (descriptor.descriptor_tag) {
                    case 0x4D:
                        ...
                        short_event.descriptor.event_name = new char_1.default(descriptor.event_name_char).decode();
                        short_event.descriptor.text = new char_1.default(descriptor.text_char).decode();
                        break;
                    case 0x4E:
                        let extended_event = descriptors.extended_event;
                        if (!extended_event.needUpdate)
                            break;
                        ...
                        extended_event.descriptor.text = new char_1.default(descriptor.text_char).decode();
                            ...
                            if (descriptor.descriptor_number === descriptor.last_descriptor_number) {
                                extended_event.descriptor.items = extended_event.descriptor.items.map(_item => {
                                    if (!Array.isArray(_item))
                                        return _item;
                                    return new char_1.default(buffer_1.Buffer.concat(_item)).decode();
                                }).join("");
                            }
                        });
                        break;
                    case 0x50:
                        ...
                        component.descriptor.text = new char_1.default(descriptor.text_char).decode();
                        component.descriptor.component_text = epg_table_1.default.component.hasOwnProperty(descriptor.stream_content) &&
                            epg_table_1.default.component[descriptor.stream_content].hasOwnProperty(descriptor.component_type) ?
                            epg_table_1.default.component[descriptor.stream_content][descriptor.component_type] : "";
                        break;
                    case 0x54:
                        ...
                        content.descriptor.contents = descriptor.contents.map(_content => {
                            return {
                                ...
                                content_nibble_level_1_text: epg_table_1.default.nibble.level_1[_content.content_nibble_level_1],
                                content_nibble_level_2_text: epg_table_1.default.nibble.level_2[_content.content_nibble_level_1][_content.content_nibble_level_2]
                            };
                        });
                        break;
                    case 0xC4:
                        let audio_component = descriptors.audio_component;
                        if (!audio_component.needUpdate)
                            break;
                        ...
                        audio_component.descriptor.text_char = new char_1.default(descriptor.text_char).decode();
                        audio_component.descriptor.dialog_control_text = epg_table_1.default.dialog_control[descriptor.component_type >> 7];
                        audio_component.descriptor.audio_for_handicapped_text = epg_table_1.default.audio_for_handicapped[(descriptor.component_type >> 5) & 0x03];
                        audio_component.descriptor.audio_mode_text = epg_table_1.default.audio_mode[descriptor.component_type & 0x1F];
                        audio_component.descriptor.quality_indicator_text = epg_table_1.default.quality_indicator[descriptor.quality_indicator];
                        audio_component.descriptor.sampling_rate_text = epg_table_1.default.sampling_rate[descriptor.sampling_rate];
                        break;
                    case 0xD6:
                        let event_group = descriptors.event_group;
                        if (!event_group.needUpdate)
                            break;
            ...
            for (let key of Object.keys(descriptors)) {
                ...
                if (descriptor.needUpdate === false)
                    continue;
